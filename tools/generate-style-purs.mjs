...
p("-- | Styles of modification/colors applied to the chalk.");
p("-- |");
p("-- | These styles are basically of function `Chalk -> Chalk`, appending another");
p("-- | modification to the given chalk.");
p("-- | The full list of styles can be found [here](https://github.com/chalk/chalk#styles)");
p("-- | in the original README.");
p("-- |");
...
p("import Chalk.Common (Chalk)");
p("import Chalk.Color (Color(..))");
p("");
p("type Style = Chalk -> Chalk");
p("");
p("{- Modifiers -}");
p("")
...
p("")
p("-- | Modify foreground color with the given color.");
p("fg :: Color -> Style");
p("fg (RGB { r, g, b }) = runFn3 _rgb r g b");
p("fg (Hex hex) = _hex hex");
p("fg (ANSI256 i) = _ansi256 i");
p("")
p("-- | Modify background color with the given color.");
p("bg :: Color -> Style");
