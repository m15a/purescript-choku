module Test.Level2 (level2Spec) where
...
level2Spec :: Spec Unit
level2Spec = describe "Chalk of level 2" do

  it "prints level 2 colors" do
    let
      c = mkChoku Has256Colors
    ...
    ("fgRGB" &| fg (RGB { r: 1, g: 2, b: 3 }) c) `shouldEqual`
      "\x1B[38;5;16mfgRGB\x1B[39m"
    ("bgRGB" &| bg (RGB { r: 1, g: 2, b: 3 }) c) `shouldEqual`
      "\x1B[48;5;16mbgRGB\x1B[49m"
    ("fgHex" &| fg (Hex "#0A0B0C") c) `shouldEqual` "\x1B[38;5;16mfgHex\x1B[39m"
    ("bgHex" &| bg (Hex "#0A0B0C") c) `shouldEqual` "\x1B[48;5;16mbgHex\x1B[49m"
