module Test.Level1 (level1Spec) where
...
level1Spec :: Spec Unit
level1Spec = describe "Chalk of level 1" do

  it "prints level 1 colors" do
    let
      c = mkChoku HasBasicColors
    ...
    ("fgRGB" &| fg (RGB { r: 1, g: 2, b: 3 }) c) `shouldEqual` "\x1B[30mfgRGB\x1B[39m"
    ("bgRGB" &| bg (RGB { r: 1, g: 2, b: 3 }) c) `shouldEqual` "\x1B[40mbgRGB\x1B[49m"
    ("fgHex" &| fg (Hex "#0A0B0C") c) `shouldEqual` "\x1B[30mfgHex\x1B[39m"
    ("bgHex" &| bg (Hex "#0A0B0C") c) `shouldEqual` "\x1B[40mbgHex\x1B[49m"
